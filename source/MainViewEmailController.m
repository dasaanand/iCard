...
@implementation MainViewEmailController
...
- (void)initname : (NSInteger) i {
	...
	if (sqlite3_prepare_v2(db, [[NSString stringWithFormat:@"select * from vctbl order by off1email, off2email, persemail;"]UTF8String], -1, &statement, NULL) == SQLITE_OK)
	{	
		while (sqlite3_step(statement) == SQLITE_ROW) 
		{
			NSString *name1 = [[NSString alloc] initWithUTF8String:(char *)(sqlite3_column_text(statement, i))];
			if (![name1 isEqualToString:@"xxx@yyy.com"]) {
...
{
	NSString *header = [[NSString alloc] initWithFormat:@"Email : Name"];
	return header;
	[header release];
}
...
- (void)changeScope {
	UIActionSheet *menu = [[UIActionSheet alloc] initWithTitle: @"Select the field for search" delegate:self cancelButtonTitle:@"Cancel" destructiveButtonTitle:@"Email" otherButtonTitles:@"City/Country",@"Contact number",@"Visting Cards", nil];
