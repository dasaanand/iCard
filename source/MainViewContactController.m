...
@implementation MainViewContactController
...
- (void)initname : (NSInteger) i {
	...
	if (sqlite3_prepare_v2(db, [[NSString stringWithFormat:@"select * from vctbl order by off1no, off2no, persno;"]UTF8String], -1, &statement, NULL) == SQLITE_OK)
	{	
		while (sqlite3_step(statement) == SQLITE_ROW) 
		{
			...
			switch (i) {
				case 19:
					if (![name1 isEqualToString:@"personal"]) {
						flag=1;
					}
					break;
				case 22:
					if (![name1 isEqualToString:@"fax"]) {
						flag=1;
					}
					break;
				default:
					if (![name1 isEqualToString:@"office"]) {
						flag=1;
					}
					break;
			}
			...
			if (flag==1) {
				...
				NSString *fullName = [name1 stringByAppendingFormat:@": %@ %@",name3,name4];
...
{
	NSString *header = [[NSString alloc] initWithFormat:@"Contact Number : Name"];
	return header;
	[header release];
}
...
- (void)changeScope {
	UIActionSheet *menu = [[UIActionSheet alloc] initWithTitle: @"Select the field for search" delegate:self cancelButtonTitle:@"Cancel" destructiveButtonTitle:@"Contact Number" otherButtonTitles:@"City/Country",@"Email",@"Visting Cards", nil];
