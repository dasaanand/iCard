...
@implementation MainViewOrgController
...
- (void)viewDidLoad {
	...
	if (sqlite3_prepare_v2(db, [[NSString stringWithFormat:@"select * from vctbl order by orgname;"]UTF8String], -1, &statement, NULL) == SQLITE_OK)
	{	
		while (sqlite3_step(statement) == SQLITE_ROW) 
		{
			NSString *name1 = [[NSString alloc] initWithUTF8String:(char *)(sqlite3_column_text(statement, 3))];
			NSString *name2 = [[NSString alloc] initWithUTF8String:(char *)(sqlite3_column_text(statement, 1))];
			NSString *name3 = [[NSString alloc] initWithUTF8String:(char *)(sqlite3_column_text(statement, 2))];

			if ([name1 isEqualToString:@"organization name"]) {
				name1=@"Unknown";
			}
			if ([name3 isEqualToString:@"last name"]) {
				name3=@"";
			}
			
			NSString *fullName = [name1 stringByAppendingFormat:@", %@ %@",name2,name3];
...
{
	NSString *header = [[NSString alloc] initWithFormat:@"Organization , Name"];
	return header;
	[header release];
}
